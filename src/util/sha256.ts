...
export default function Sha256(inputMessage: string): string {
  function hash(inputMessage: string) {
    let msg = Utf8Encode(inputMessage);
    ...
    for (let h = 0; h < H.length; h++) {
      result += `00000000${H[h].toString(16)}`.slice(-8);
    }
